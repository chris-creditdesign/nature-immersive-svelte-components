...
<script lang="ts">
	...
	import type { Space, AlignItems } from 'creditdesign-svelte-components/types';
	...
	export let articles: { title: string; url: string; doi: string; published: boolean }[];
	...
	export let stand: string;
	export let stackSpace: Space = 'var(--s0)';
	export let headerStackSpace: Space = 'var(--s-3)';
	export let boxSpace: Space = 'var(--s-1)';
	/**
	 * Flexbox align-items between headline/standfirst and button group.
	 */
	export let alignItems: AlignItems = 'flex-start';
	/**
...
<style>
	...
	.list-item--current {
		padding-left: 10px;
		border-left: 5px solid var(--color--link--component, var(--color--link--global));
	}
</style>
			...
			<Cluster {alignItems}>
				<div class="cluster__grow">
					<Stack stackSpace={headerStackSpace}>
						<Header text={headline} {headerLevel} {headlineFontSize} {headlineFontWeight} />
				...
				{#if mounted}
					<ExpandButton {expanded} {message} {expandedMessage} {theme} on:click={handleClick} />
				{/if}
			</Cluster>
