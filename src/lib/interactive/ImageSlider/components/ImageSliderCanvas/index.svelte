<script lang="ts">
	...
	// A frame variable which stores the request animation frame
	// return value (a long interger) so we can cancel it when the component is destroyed
	let frame: number;
	...
	let loop = () => {
		...
		if (amountToReveal > 0) {
			...
			ctx.drawImage(imageA, 0, 0, imageA.width, imageA.height, 0, 0, width, height);
		}
