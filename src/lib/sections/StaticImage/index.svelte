...
-->
<script lang="ts">
	import type { StaticImageStep } from '$lib/types';
	 ...
	 */
	export let steps: StaticImageStep[];
	/**
	...
	let textWidthComponent = textWidth ? `--static-image-text-width--component: ${textWidth};` : '';

	let imageWidthComponent = imageWidth
		? `--static-image-image-width--component: ${imageWidth};`
		: '';

	let gridGapComponent = gridGap ? `--static-image-grid-gap--component: ${gridGap};` : '';
	...
	onMount(() => {
		...
		const onEnterScreen = (entries: IntersectionObserverEntry[]) => {
			entries
		...
		if (textContainer) {
			const renderedSteps = Array.from(textContainer.querySelectorAll('[data-index]'));
