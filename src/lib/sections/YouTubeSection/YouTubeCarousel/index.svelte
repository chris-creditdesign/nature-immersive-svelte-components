<script lang="ts">
	...
	import LiteYouTube from '$lib/videos/LiteYouTube/index.svelte';
	import VideoButton from '$lib/sections/YouTubeSection/SelectVideoButton/index.svelte';
	import VideoInfo from '$lib/sections/YouTubeSection/VideoInfo/index.svelte';

	export let videoDataArray: VideoData[];
	export let headerLevel = 'h2';
	export let headlineFontSize = 'big-1';
	export let videoListLabel = 'Choose a video from the following list to load in the above player.';
	...
	let requestVideo = async (event: CustomEvent<{ index: number }>) => {
...
>
	<svelte:fragment slot="main-content">
		...
		>
			<div role="region" aria-live="polite" aria-atomic="true" class="visually-hidden">
				<p>
					Video {currentVideoIndex + 1} of {videoDataArray.length} selected.
				</p>
			</div>

			<LiteYouTube bind:youTubeContainer {videoId} {title} {playVideoRequested} autoplay={false} />
