...
<script lang="ts">
	 ...
	 */
	export let frameRatioHeight: number;
	export let frameRatioWidth: number;
	/**
	...
	let buttonVerticalPosition = buttonAtTop ? `--video-button-row--component: 2 / 3;` : '';

	let buttonHorizontalPosition = buttonOnLeft ? `--video-button-column--component: 2 / 3;` : '';

	let videoCaptionSpace = captionSpace ? `--video-caption-space--component: ${captionSpace};` : '';
...
<style>
	...
	:global(.video-button) {
		grid-column: var(--video-button-column--component, var(--video-button-column--global, 4 / 5));
		grid-row: var(--video-button-row--component, var(--video-button-row--global, 4 / 5));
	}
...
<figure class={`${className}`}>
	...
	{#if caption.length}
		<figcaption class="font-size:small font-family:sans-serif" style={captionStyle}>
			{@html caption}
			{#if !mounted}
				Here is a
